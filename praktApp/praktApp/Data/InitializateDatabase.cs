    ...
    class InitializateDatabase
    {
        public static async void  InitializateDB()
        {
            if (!File.Exists(Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.LocalApplicationData), "ElectronicBookDB.db3")))
            {
                ...
                CompleteCategory completeCategory5 = new CompleteCategory() { Category = category1, User = user2, IsChoose = false, IsStuded = false, CanChange = true };
                CompleteCategory completeCategory6 = new CompleteCategory() { Category = category2, User = user2, IsChoose = false, IsStuded = false, CanChange = true };
